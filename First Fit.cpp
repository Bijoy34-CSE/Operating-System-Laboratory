...
// Function to add a new process using First Fit
void addNewProcess_First(vector<MemoryBlock>& memory, string newProcessName, long long newProcessSize, string unit) {
    ...
    for (int i = 0; i < memory.size(); i++) {
        if (!memory[i].allocated && memory[i].size >= newProcessSizeBytes) {
            long long remainingSize = memory[i].size - newProcessSizeBytes;

            memory[i].allocated = true;
            memory[i].process = newProcessName;
            memory[i].size = newProcessSizeBytes;
            memory[i].end = memory[i].start + newProcessSizeBytes - 1;

            if (remainingSize > 0) {
                memory.insert(memory.begin() + i + 1, {remainingSize, false, "", memory[i].end + 1, memory[i].end + remainingSize});
            }
            allocated = true;
            break;
        }
    ...
    if (allocated) {
        cout << "\nProcess " << newProcessName << " with size " << newProcessSize << " allocated successfully.\n";
        displayGanttChart(memory);
    } else {
        cout << "\nProcess " << newProcessName << " could not be allocated. Not enough space.\n";
    }
}
...
int main() {
    ...
    while(r){
        ...
        addNewProcess_First(memory, newProcessName, newProcessSize, memoryUnit);
        cout<<"1. Add new process"<<endl;
        cout<<"0. Exit"<<endl;
        //int r;
        cin>>r;
